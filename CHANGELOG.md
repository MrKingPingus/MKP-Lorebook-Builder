...
- **Every per-entry action now lives in a ⋯ menu on the entry's header** — and works without opening the entry. **Copy to lorebook**, **Move to lorebook**, **Move to folder**, **Public on CharSnap**, **Hide from Export** and **Delete entry** are all one click from a collapsed entry. Most of them used to be spread across a footer that only an expanded entry shows, so acting on an entry while skimming meant expanding it first. **Remove** is gone from the header; it is **Delete entry** at the bottom of the menu, and **Ctrl+Z** still brings the entry back. Checkpoints stay in the entry itself, where their panel opens.
