...
- **More things you can bind** — alongside New Entry, Undo, and Redo, you can now set shortcuts for toggling Select mode, expand/collapse all, focusing search, opening Find & Replace, toggling and swapping the reference panel, exporting, importing entries, opening Settings, and opening the shortcut list. A few more (per-entry snapshot and compare) are marked “soon” because they need a keyboard-focused entry to act on — that's coming with the accessibility pass.
