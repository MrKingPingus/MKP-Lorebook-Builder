...
- [x] **Wired actions (13)** — new entry, undo, redo, toggle select, expand/collapse all, focus search, focus find & replace, toggle reference, swap reference (crosstalk), export, import entries, open settings, keyboard help. Still reserved (collision-checked defaults): **save snapshot** and **toggle compare** — both need a "current entry" to act on, which waits on the deferred focused-entry/roving-cursor work; and **next/prev cross-match**, which needs a match-cursor (a small feature, not a wire-up).
...
3. ✅ **Wider configurable set + dispatch table** — done. Actions are registry entries; 13 are wired. Only three remain reserved: **save snapshot** and **toggle compare** (blocked on the focused-entry concept — no "current entry" to target on desktop) and **next/prev cross-match** (needs a match-cursor).
