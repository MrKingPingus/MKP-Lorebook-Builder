...
Phases with locked design decisions, as opposed to Future Features which are still ideas. Some have since shipped and stay here with their decisions intact — status is marked inline on each phase's sub-phase line, not by which section it sits in. As of 2026-08-16: **11 complete**, **12 not started**, **13 complete**, **14 in progress (14A–14D complete; 14E scoped, not started)**. 11 and 12 came out of the 2026-07-24 "what's next" user-poll planning pass; Folders is the poll front-runner, Templates the runner-up, sequenced so Templates can reuse Folders' category primitive.
...
2. **The Back gesture leaves the site rather than closing a full-screen layer** — there is no `pushState`/`popstate` handling anywhere in `src/`, so opening the entry editor adds no history entry (measured: `history.length` 2 before, 2 after) and Back lands on `about:blank`. Each layer's own dismissal control works; the platform gesture for "close this full-screen view" is unhandled. **Deferred 2026-08-11 — to revisit, not dropped.** Making full-screen layers history entries has its own costs, and it is a design decision rather than a patch. The suite records the current behaviour as a quirk, so if it ever changes by accident the roll-up says so.
...
- **14B — Navigation spine · complete (2026-08-14).** The mobile title menu (two tabs, per decision 1), the reference chooser, the touch-floor tokens, the breakpoint rule, the hotbar action, the lander route. Fixes #123 at the root and gives create/delete/rename a home. **Design approved 2026-08-12** against `mockup-mobile-overhaul.html` (seven states, same throwaway spirit as `mockup-ui-overhaul.html`) — the mockup is the reference for decisions 7–13 and should be deleted with `docs/mobile-findings.md` once the phase lands. What it produced, and the two things worth carrying forward:
