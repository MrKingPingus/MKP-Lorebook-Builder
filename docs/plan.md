...
A Settings toggle ("Look up synonyms via the dictionary API", default **on**) gates a chip-anchored synonym popover on the suggestions tray. Hovering a suggestion chip (desktop) or long-pressing it (mobile, ~450ms) opens a portal popover with synonyms grouped by part of speech — a header with `◀ ▶` cycle arrows lets the user pick which PoS context the synonyms should reflect (e.g., "good" as an adjective vs. "good" as a noun). The popover is multi-select — the user toggles synonym chips on, then clicks **Add** in the header to commit all selected words to the entry's triggers field at once. Selection persists across sense changes within the same word so a user can pull from one PoS, cycle to another, pick more, and Add everything in one commit. The synonym list is rendered as a single scrollable container (no pagination); selected chips remain visible regardless of scroll position. Words with no available senses or a 404 land on a "No synonyms found" empty state. Data source: `dictionaryapi.dev` (free, no API key, CORS-friendly, wraps WordNet/Wiktionary). Synonyms are pulled from the meaning-level (PoS) array — the per-definition synonym field exists in the schema but is almost always empty in practice, so the granularity is per-PoS rather than per-individual-sense. Senses with empty synonym lists are dropped; remaining senses capped to the top 5 by `THESAURUS_SENSE_CAP`. The dictionary indexes lemmas (base forms), so inflected words (`features`, `lives`, `majoring`, `running`, `faster`, etc.) typically 404; a small rule-based lemmatizer (`lemmatize.js`) generates base-form candidates and the service retries each in priority order on an empty original lookup. Per-word cache lives at module scope in `use-thesaurus.js` (in-memory only, no `localStorage` — keeps storage pressure off the rollback/autosave budget). New files: `thesaurus-service.js`, `use-thesaurus.js`, `ThesaurusPopover.jsx`. Modified: settings store/hook/panel + `SuggestionsTray.jsx` (hover + long-press handlers, popover mount) + `limits.js` (`THESAURUS_SENSE_CAP = 5`, `THESAURUS_LONG_PRESS_MS = 450`). The Settings hint discloses that the feature makes outbound requests to `api.dictionaryapi.dev`. Mobile discoverability relies on the long-press; the Settings hint mentions it. Depends on: nothing blocking.
