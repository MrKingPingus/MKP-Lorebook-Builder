...
- **14B — Navigation spine.** The mobile title menu (two tabs, per decision 1), the reference chooser, the touch-floor tokens, the breakpoint rule, the hotbar action. Fixes #123 at the root, restores the lander route, gives create/delete/rename a home. Also the harness work: a `pairCrosstalkMobile` driver pathway and popover poses in the sweep, so the surface is testable *before* it is rebuilt. **Design approved 2026-08-12** against `mockup-mobile-overhaul.html` (seven states, same throwaway spirit as `mockup-ui-overhaul.html`) — the mockup is the reference for decisions 7–13 and should be deleted with `docs/mobile-findings.md` once the phase lands.
