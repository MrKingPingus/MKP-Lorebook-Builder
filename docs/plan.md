...
A Settings toggle ("Look up synonyms via Datamuse", default **on**) gates a chip-anchored synonym popover on the suggestions tray. Hovering a suggestion chip (desktop) or long-pressing it (mobile, ~450ms) opens a portal popover with up to 5 synonym mini-chips fetched from Datamuse `rel_syn` (strict synonyms only, free, no API key). The popover is multi-select — the user toggles synonym chips on, then clicks **Add** in the header to commit all selected words to the entry's triggers field at once. A "More ▾" button pages through the cached results 5 at a time and disappears when the pool is exhausted. Proper nouns and multi-word phrases (which dominate the suggestion pool) land on a "No synonyms found" empty state — the popover still opens, but with no chips. Per-word cache lives at module scope in `use-thesaurus.js` (in-memory only, no `localStorage` — keeps storage pressure off the rollback/autosave budget). New files: `thesaurus-service.js`, `use-thesaurus.js`, `ThesaurusPopover.jsx`. Modified: settings store/hook/panel + `SuggestionsTray.jsx` (hover + long-press handlers, popover mount) + `limits.js` (`THESAURUS_PAGE_SIZE = 5`). The Settings hint discloses that the feature makes outbound requests to `api.datamuse.com`. Mobile discoverability relies on the long-press; the Settings hint mentions it. Depends on: nothing blocking.
