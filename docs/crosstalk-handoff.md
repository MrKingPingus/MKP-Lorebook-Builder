...
Phase D1 (and the D1.1 match-counter polish) is verified in browser. Counter shows `Active N · Reference N` in crosstalk mode; find/replace popover groups counts per-book; swap preserves terms. Move on to **D2**.

**D2** — two Apply buttons ("Apply to Active", "Apply to Reference"). Apply-to-reference swaps, applies, swaps back — all state flips, no remount. The hook's current `replaceAll` is active-only; D2 adds a mirror path that uses the store swap to temporarily swing the active id onto the reference book, runs `findReplace` via `updateActiveEntries`, then swaps back. Make sure the history snapshot captures the pre-state for whichever book actually mutates (the snapshot is keyed off `activeLorebookId` at the moment of `pushSnapshot`, so the temporary swap should happen *before* the snapshot, not after). Plumb both buttons through `FindReplace.jsx` — the popover currently has one Proceed button; in crosstalk it should split into two, gated on per-book match counts (disable each independently when its book has 0 matches). Revisit the open question on autosave (#1) after a real session of swap+apply — that's the simplest way to surface any staleness.
