...
- **C1.5 polish (done)**: (a) Swap now keeps the clicked panel in its slot. Added `activeSide: 'left' | 'right'` + `toggleActiveSide` to `ui-store`; `useReferenceLorebook.swapReference` flips both the id roles and the side flag. `FloatingWindow` picks which component renders in each pane-split slot based on `activeSide`, so only the role changes visually — no left↔right jump. (b) Reference content dimmed: `.reference-panel-name` and `.reference-panel-body` fade to `opacity: 0.62`, with a hover fade back to 1 as an "edit here if you click" cue. The picker stays full-opacity. (c) The pane divider moved off `.reference-panel.border-left` to a `.pane-split-slot + .pane-split-slot` between-slot border so the divider is positional, not content-tied. Future: when diff highlighting lands, the same "matching strings on both sides" treatment should extend to search-term highlights.
