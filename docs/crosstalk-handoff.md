...
Phase D1 is done. Load the app with `?crosstalk=1`, pick a reference lorebook, switch Search to Find/Replace, type a term that appears in both books, and verify: the popover shows a per-lorebook list with correct counts above the scope chips; the main button count is the sum across both books; single-panel mode (no ref set, or no `?crosstalk=1`) hides the per-book list and looks byte-identical to before; swapping active↔reference keeps the current find/replace terms (because the sorted id set didn't change); switching the active picker to a book not in the reference slot still resets the fields.

Then start **D2** — two Apply buttons ("Apply to Active", "Apply to Reference"). Apply-to-reference swaps, applies, swaps back — all state flips, no remount. The hook's current `replaceAll` is active-only; D2 will add a mirror path that uses the store swap to temporarily swing the active id onto the reference book, run `findReplace` via `updateActiveEntries`, then swap back. Make sure the history snapshot still captures the pre-state for whichever book actually mutates. Revisit the open question on autosave (#1) after a real session of swapping — that's the simplest way to surface any staleness.
