...
| `ui-store.js` | active tab, search query, type filter, `folderFilter` (folder ids + the `UNFILED_FILTER_ID` sentinel; active-book only, pruned on read via `use-folder-filter`), window pos/size, `bulkExpanded` + `expandAllNonce`/`collapseAllNonce` (Expand/Collapse All pulses), `groupByType` |
